...
enum class Category {
    ...
    String,         // строковый литерал (заменяется целиком, с кавычками)
    StringFragment, // текстовый сегмент внутри интерполированной строки
                    // (Python f-string, JS template literal): заменяется
                    // «на месте», без обрамляющих кавычек
