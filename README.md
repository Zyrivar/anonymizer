...
| Python | `PythonLanguageProfile` | комментарии `#`, без extern, preserve builtins |
...
| `test_e2e`  | end-to-end тесты, C++ и Python (95 проверок) |
...
- **test_e2e** (95 проверок) — реалистичный SCADA-исходник C++ ~335 строк через полный
  конвейер: анонимизация, восстановление, аудит утечек, межфайловая консистентность;
  плюс блок Python: round-trip, устранение данных, preserve builtins, `#`-комментарии,
  изоляция профилей.
...
- Языки — C/C++ и Python (прототип). Python-профиль: preserve-лист builtins/stdlib
  стартовый; f-string-интерполяции анонимизируются как строка целиком. Импорты —
  обычные имена (модули → `ID_*`). Форматы данных вне scope.
...
локализация UI (ru/en) → **многоязычность** (профили языков, прототип Python).
